...
# ファイル読み込み（3次元ベクトル時系列をそのまま返す）
def load_dat(filepath):
    try:
        data = np.loadtxt(filepath)
        if data.ndim != 2 or data.shape[1] != 3:
            raise ValueError(f"データ形式が (T, 3) ではありません: {filepath}, shape={data.shape}")
        return [vec for vec in data]  # 各行が1時刻の3次元ベクトル
